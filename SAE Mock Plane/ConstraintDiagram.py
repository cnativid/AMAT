...
def Constraint_Cruise(WS,V_variation):
    eta_p = 0.65 # prop efficiency at trim
    C_Lmax = 1.5
    P_cruise = 0.5 # frac of max power for cruise
    V_stall = np.sqrt(2*WS/rho/C_Lmax)
    V_cruise = V_stall + V_variation
    # LD = 9 # trim L/D
    C_D0 = 0.02
    q  = rho/2*V_cruise**2
    AR = 10
...
if __name__ == "__main__":
    ...
    WS_Landing = Constraint_LDG(200)*np.ones(N)
    ...
    V_variation = [0,10,20,30,40,50] # Difference between cruise velocity and stall velocity
    # V_Cruise = LDG_values[1]*np.ones(len(V_variation)) + V_variation
    WP_Cruise = np.zeros([len(V_variation),len(WS)])
    for i in range(0,len(V_variation)):
        WP_Cruise[i,:] = Constraint_Cruise(WS,V_variation[i]) # Cruise @ V_stall + V_variation


    ## Plot constraints
    plt.plot(WS,Constraint_TO(WS), label = '100 ft Takeoff')
    plt.plot(WS_Landing,WP, label = '200 ft Landing')
